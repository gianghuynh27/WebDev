...
export async function getDbStaticChampionsController(
_req: Request,
res: Response,
) {
try {
  const result = await getDbStaticChampions();
...
export async function getDbStaticItemsController(_req: Request, res: Response) {
try {
  const result = await getDbStaticItems();
  res.json(result);
} catch {
...
export async function getDbStaticAugmentsController(
_req: Request,
res: Response,
) {
try {
  const result = await getDbStaticAugments();
  res.json(result);
} catch {
  res.status(500).json({ message: "Failed to load static augments" });
}
}
export async function getDbStaticTraitsController(
_req: Request,
res: Response,
) {
try {
  const result = await getDbStaticTraits();
